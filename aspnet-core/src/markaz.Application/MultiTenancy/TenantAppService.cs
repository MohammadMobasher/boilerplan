    ...
    public class TenantAppService : AsyncCrudAppService<Tenant, TenantDto, int, PagedTenantResultRequestDto, CreateTenantDto, TenantDto>, ITenantAppService
    {
        ...
        public async override Task<PagedResultDto<TenantDto>> GetAllAsync(PagedTenantResultRequestDto input)
        {
            var count = await this.Repository.CountAsync();
                //_tenantManager.Tenants.Count();
            var tenants = await _tenantManager
                .Tenants
                .PageBy(input)
                .ToListAsync();

            return new PagedResultDto<TenantDto>(count, ObjectMapper.Map<List<TenantDto>>(tenants));
        }
